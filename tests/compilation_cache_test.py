...
class CompilationCacheTest(jtu.JaxTestCase):
  ...
  def test_compile_requests_use_cache_metric(self):
    ...
    self.assertEqual(
        ...
        - counts["/jax/compilation_cache/compile_requests_use_cache"],
        3)

  def test_caching_skipped_metric(self):
    counts = Counter(_counts)
    with tempfile.TemporaryDirectory() as tmpdir, persistent_cache_min_compile_time_secs(
        2):
      cc.initialize_cache(tmpdir)

      # Mock time to create a short compilation time, cache skipped.
      with mock.patch("time.monotonic", side_effect=np.arange(0, 1, 0.1)):
        jit(lambda x: x + 1)(1)

    self.assertEqual(
        _counts["/jax/compilation_cache/caching_skipped"]
        - counts["/jax/compilation_cache/caching_skipped"],
        1)
  ...
  def test_cache_hits_original_metric(self):
    ...
    self.assertEqual(
        ...
        - counts["/jax/compilation_cache/cache_hits_original"],
        1)
