...
//     data region.
// TODO(tlongeri): Isn't TiledRectangularVregBounds more general?
...
class VectorLayout {
 ...
 private:
  ...
  void verify(std::array<int64_t, 2> target_shape) const {
    CHECK(!offsets_[0].has_value() ||
          0 <= offsets_[0] && offsets_[0] < packing() * target_shape[0]);
    CHECK(!offsets_[1].has_value() ||
          0 <= offsets_[1] && offsets_[1] < target_shape[1]);
    // Tiling should neatly divide the target shape, so that every vector
    // register ends up having the same structure.
    CHECK_EQ(packing() * target_shape[0] % tiling_[0], 0);
    CHECK_EQ(target_shape[1] % tiling_[1], 0);
    CHECK(hasNativeTiling(target_shape) ||
          llvm::all_of(offsets_, [](auto o) { return o.has_value(); }));
  }
...
// TODO(tlongeri): Move the following to a util file
template <typename T>
...
inline arith::ConstantOp IdxCst(int64_t idx, OpBuilder &builder, Location loc) {
