...
LogicalResult elementwise_op_rule(
    RewriteContext &ctxt, Operation &op, const ArrayRef<Layout> layouts_in,
  ...
  for (unsigned i = 0; i < num_operands; ++i) {
    FAILUREOR_ASSIGN_OR_RETURN(
        ...
        disassemble(ctxt, *layouts_in[i], op.getOperand(i)));
...
LogicalResult elementwise_op_rule_unpacked(RewriteContext &ctxt, Operation &op,
  ...
  return elementwise_op_rule_unpacked_impl(
      ctxt, op, layouts_in, layouts_out, std::move(factory),
...
LogicalResult arith_cmpf_rule(RewriteContext &ctxt, Operation &op,
  ...
  return elementwise_op_rule_unpacked<2>(
      ctxt, op, layouts_in, layouts_out,
      [&](RewriteContext &ctxt, const Value lhs,
...
LogicalResult arith_cmpi_rule(RewriteContext &ctxt, Operation &op,
                              const ArrayRef<Layout> layouts_in,
                              const ArrayRef<Layout> layouts_out) {
...
LogicalResult arith_extui_rule(RewriteContext &ctxt, Operation &op,
                               const ArrayRef<Layout> layouts_in,
                               const ArrayRef<Layout> layouts_out) {
  ...
  return elementwise_op_rule(
      ...
      [&](RewriteContext &ctxt,
        ...
        const Value x = operands[0];
        const VectorType x_ty = cast<VectorType>(x.getType());
        const VectorType out_ty = VectorType::get(x_ty.getShape(), elem_ty);
        return ctxt.builder.create<arith::ExtUIOp>(extui_op.getLoc(), out_ty, x)
            .getOperation();
      });
}

template <typename OpTy>
LogicalResult ext_op_rule_impl(RewriteContext &ctxt, OpTy op,
  ...
  FAILUREOR_ASSIGN_OR_RETURN(const xla::Array<Value> input_vregs,
                             disassemble(ctxt, layout_in, op.getIn()));
  xla::Array<Value> output_vregs(
      layout_out.tileArrayShape(result_ty.getShape(), ctxt.target_shape));
  FAILUREOR_ASSIGN_OR_RETURN(
      const VectorType res_vreg_ty,
      getNativeVregType(result_ty.getElementType(), ctxt.target_shape));
  if (layout_in.implicit_dim() != layout_out.implicit_dim()) {
    return op.emitOpError("Not implemented: Change of layout furing the cast");
  }
  switch (layout_in.implicit_dim()) {
    case VectorLayout::ImplicitDim::kNone: {
      if (layout_in.tiling() != ctxt.target_shape ||
          layout_out.tiling() != ctxt.target_shape) {
      ...
      output_vregs.Each([&](absl::Span<const int64_t> idxs, Value *v) {
        ...
        *v = ctxt.builder.create<UnpackSubelementsOp>(
    ...
    case VectorLayout::ImplicitDim::kSecondMinor: {
      ...
      if (layout_in.offsets()[0] >= ctxt.target_shape[0]) {
        return op.emitOpError("Not implemented");
      }
      auto unpack_subelements_op = ctxt.builder.create<UnpackSubelementsOp>(
          op.getLoc(), res_vreg_ty, *input_vregs.begin(), 0);
      output_vregs.Fill(unpack_subelements_op.getResult());
    }
  }
  op.replaceAllUsesWith(
      assemble(ctxt, result_ty, layout_out, std::move(output_vregs))
          .getResult());
  op.erase();
  return success();
}

LogicalResult arith_extf_rule(RewriteContext &ctxt, Operation &op,
                              const ArrayRef<Layout> layouts_in,
                              const ArrayRef<Layout> layouts_out) {
  ...
  return ext_op_rule_impl(ctxt, extf_op, *layouts_in.front(),
                          *layouts_out.front());
}

LogicalResult arith_extsi_rule(RewriteContext &ctxt, Operation &op,
                               const ArrayRef<Layout> layouts_in,
                               const ArrayRef<Layout> layouts_out) {
  CHECK_EQ(layouts_in.size(), 1);
  CHECK(layouts_in.front().has_value());
  ...
  return ext_op_rule_impl(ctxt, extsi_op, *layouts_in.front(),
                          *layouts_out.front());
}

template <typename OpTy>
LogicalResult trunc_op_rule_impl(RewriteContext &ctxt, OpTy op,
                                 const VectorLayout &layout_in,
                                 const VectorLayout &layout_out) {
  auto result_ty = cast<VectorType>(op.getResult().getType());
  FAILUREOR_ASSIGN_OR_RETURN(const xla::Array<Value> input_vregs,
  ...
  if (layout_in.implicit_dim() == VectorLayout::ImplicitDim::kNone &&
    ...
    if (layout_out.tiling() == ctxt.target_shape) {
      const int packing = layout_out.packing();
      output_vregs.Each([&](absl::Span<const int64_t> idxs, Value *v) {
        SmallVector<Value> parts;
        const int64_t col_base = idxs.back() * packing;
        for (int64_t i = 0; i < packing; ++i) {
          // Pack any data lying around if OOB
          SmallVector<int64_t> idxs_local(toArrayRef(idxs));
          idxs_local.back() =
              std::min(col_base + i, input_vregs.dimensions().back() - 1);
          parts.push_back(input_vregs(idxs_local));
        }
        *v = ctxt.builder.create<PackSubelementsOp>(op.getLoc(), res_vreg_ty,
                                                    parts);
      });
      ...
      output_vregs.Each([&](absl::Span<const int64_t> idxs, Value *v) {
        ...
        *v = ctxt.builder.create<PackSubelementsOp>(
...
LogicalResult arith_truncf_rule(RewriteContext &ctxt, Operation &op,
                                const ArrayRef<Layout> layouts_in,
                                const ArrayRef<Layout> layouts_out) {
  CHECK_EQ(layouts_in.size(), 1);
  CHECK(layouts_in.front().has_value());
  ...
  return trunc_op_rule_impl(ctxt, truncf_op, *layouts_in.front(),
                            *layouts_out.front());
}

LogicalResult arith_trunci_rule(RewriteContext &ctxt, Operation &op,
                                const ArrayRef<Layout> layouts_in,
                                const ArrayRef<Layout> layouts_out) {
  CHECK_EQ(layouts_in.size(), 1);
  CHECK(layouts_in.front().has_value());
  ...
  return trunc_op_rule_impl(ctxt, trunci_op, *layouts_in.front(),
                            *layouts_out.front());
}

LogicalResult vector_load_rule(RewriteContext &ctxt, Operation &op,
                               const ArrayRef<Layout> layouts_in,
                               const ArrayRef<Layout> layouts_out) {
  CHECK_EQ(layouts_out.size(), 1);
  MLIRContext *const mlir_ctxt = op.getContext();
  ...
  FAILUREOR_ASSIGN_OR_RETURN(
      ...
      getNativeVregType(ty.getElementType(), ctxt.target_shape));
  ...
  FAILUREOR_ASSIGN_OR_RETURN(
      ...
      getMemRefTiling(load_op.getBase(), ctxt.target_shape));
  ...
  if (offsets[0] == std::nullopt) {
    ...
    if (!layout_out.hasNativeTiling(ctxt.target_shape)) {
      return op.emitOpError("Not implemented");
    }
    // affine_map<(..., j) -> (0, j)
    load_map =
        AffineMap::get(memref_ty.getRank(), 0,
                       {getAffineConstantExpr(0, mlir_ctxt),
                        getAffineDimExpr(memref_ty.getRank() - 1, mlir_ctxt)},
                       mlir_ctxt);
    FAILUREOR_ASSIGN_OR_RETURN(const TypedAttr zero_attr,
                               getZeroFloatOrIntegerAttr(ty));
    padding = ctxt.builder.create<arith::ConstantOp>(
  ...
  xla::Array<Value> tiles(
      layout_out.tileArrayShape(ty.getShape(), ctxt.target_shape));
  const std::array<int64_t, 2> vreg_slice =
      layout_out.vregSlice(ctxt.target_shape);
  ...
  const absl::Status status =
      tiles.EachStatus([&](absl::Span<const int64_t> tile_idxs, Value * /*v*/) {
        ...
        CHECK(tile_idxs[num_dims - 1] + ctxt.target_shape[1] <=
              memref_ty.getShape()[num_dims - 1]);
        std::unique_ptr<VRegDataBounds> bounds = layout_out.tileDataBounds(
            mlir_ctxt, ty.getShape(), toArrayRef(tile_idxs), ctxt.target_shape,
            /*allow_replicated =*/{true, false});
        SmallVector<Value> idxs_vs(idxs.size());
        for (int64_t i = 0; i < idxs.size(); ++i) {
          idxs_vs[i] = IdxCst(idxs[i], ctxt.builder, load_op->getLoc());
        }
        Operation *tile;
        if (bounds->maskVariesAlong(Direction::kSublanes, ctxt.target_shape)) {
          CHECK(offsets[0].has_value());
          tile = ctxt.builder.create<tpu::LoadOp>(
              load_op.getLoc(), target_ty, load_op.getBase(), idxs_vs,
              bounds->getSublaneMask(mlir_ctxt, ctxt.target_shape), nullptr);
        } else {
          if (load_map) {
            CHECK(padding);
            if (layout_out.bitwidth() != 32) {
            ...
            tile = ctxt.builder.create<vector::TransferReadOp>(
                load_op.getLoc(), target_ty, load_op.getBase(), idxs_vs,
                load_map, padding, nullptr, nullptr);
          } else {
            const SmallVector<bool> sublane_mask(ctxt.target_shape[0], true);
            const auto sublane_mask_attr =
                DenseBoolArrayAttr::get(mlir_ctxt, sublane_mask);
            tile = ctxt.builder.create<tpu::LoadOp>(load_op.getLoc(), target_ty,
  ...
  load_op->replaceAllUsesWith(assemble(ctxt, ty, layout_out, std::move(tiles)));
  load_op->erase();
  return success();
}

LogicalResult vector_store_rule(RewriteContext &ctxt, Operation &op,
                                const ArrayRef<Layout> layouts_in,
                                const ArrayRef<Layout> layouts_out) {
  CHECK_EQ(layouts_out.size(), 1);
  MLIRContext *const mlir_ctxt = op.getContext();
  ...
  if (to_store_layout.tiling() !=
      getMemRefTiling(store_op.getBase(), ctxt.target_shape)) {
    return op.emitOpError("Not implemented");
  }
  FAILUREOR_ASSIGN_OR_RETURN(
  ...
  FAILUREOR_ASSIGN_OR_RETURN(
      xla::Array<Value> tiles,
      disassemble(ctxt, to_store_layout, store_op.getValueToStore()));
  ...
  const std::array<int64_t, 2> vreg_slice =
      to_store_layout.vregSlice(ctxt.target_shape);
  const bool can_use_vector_store =
      to_store_layout.hasNaturalTopology(ctxt.target_shape);
  const absl::Status status =
      tiles.EachStatus([&](const absl::Span<const int64_t> idx,
                           const Value tile) -> absl::Status {
        const std::unique_ptr<VRegDataBounds> bounds =
            to_store_layout.tileDataBounds(mlir_ctxt, stored_shape,
                                           toArrayRef(idx), ctxt.target_shape);
        const int64_t sidx = idx[idx.size() - 2];
        const int64_t lidx = idx[idx.size() - 1];
        SmallVector<Value> indices(ndims);
        auto boundIdxCst = std::bind(IdxCst, std::placeholders::_1,
                                     ctxt.builder, store_op->getLoc());
        for (int64_t i = 0; i < nbatchdims; ++i) {
          indices[i] = boundIdxCst(idx[i] + base_indices[i]);
        }
        if (!is_1d) {
          indices[idx.size() - 2] =
              boundIdxCst(base_s + sidx * vreg_slice[0] - *sublane_offset);
        }
        indices[idx.size() - 1] =
            boundIdxCst(base_l + lidx * vreg_slice[1] - *lane_offset);
        const DenseBoolArrayAttr sublane_mask =
            bounds->getSublaneMask(store_op->getContext(), ctxt.target_shape);
        const bool masks_subelements =
            bounds->maskVariesAlong(Direction::kSubelements, ctxt.target_shape);
        if (bounds->maskVariesAlong(Direction::kLanes, ctxt.target_shape) ||
            masks_subelements) {
          auto failure_or_mask = bounds->getVectorMask(
              ctxt.builder, ctxt.hardware_generation, ctxt.target_shape);
          ...
          if (masks_subelements) {
            auto data = ctxt.builder.create<tpu::LoadOp>(
            ...
            if (mask_is_a_bitmask) {
              auto ones = ctxt.builder.create<arith::ConstantOp>(
                  store_op->getLoc(), mask.getType(),
                  DenseElementsAttr::get(mask.getType(),
                                         ctxt.builder.getI32IntegerAttr(
                                             /*0xFFFFFFFF*/ -1)));
              auto masked_tile = ctxt.builder.create<arith::AndIOp>(
                  store_op.getLoc(), mask,
                  ctxt.builder.create<tpu::BitcastOp>(store_op.getLoc(),
                                                      mask.getType(), tile));
              auto mask_neg = ctxt.builder.create<arith::XOrIOp>(
                  store_op.getLoc(), ones, mask);
              auto masked_data = ctxt.builder.create<arith::AndIOp>(
                  store_op.getLoc(), mask_neg,
                  ctxt.builder.create<tpu::BitcastOp>(store_op.getLoc(),
                                                      mask.getType(), data));
              updated = ctxt.builder.create<tpu::BitcastOp>(
                  store_op.getLoc(), tile.getType(),
                  ctxt.builder.create<arith::OrIOp>(store_op.getLoc(),
                                                    masked_data, masked_tile));
            } else {
              updated = ctxt.builder.create<arith::SelectOp>(store_op->getLoc(),
                                                             mask, tile, data);
            }
            ctxt.builder.create<tpu::StoreOp>(
          ...
          ctxt.builder.create<vector::StoreOp>(store_op.getLoc(), tile,
...
LogicalResult applyLayoutOp(RewriteContext &ctxt, Operation &op) {
  ...
  if (auto rule_it = rules().find(op.getName().getStringRef());
    ...
    return rule(ctxt, op, layout_in, layout_out);
  }
