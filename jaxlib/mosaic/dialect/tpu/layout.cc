...
FailureOr<TypedValue<VectorType>> RectangularVregBounds::getVectorMask(
    ...
    const std::array<int64_t, 2> target_shape) const {
  auto boundIdxCst = std::bind(IdxCst, std::placeholders::_1, builder,
  ...
  return cast<TypedValue<VectorType>>(
      builder
          .create<tpu::CreateMaskOp>(
              ...
              /*low=*/
              ValueRange{boundIdxCst(starts_[0]), boundIdxCst(starts_[1])},
              /*high=*/ValueRange{boundIdxCst(ends_[0]), boundIdxCst(ends_[1])})
          .getResult());
}
...
llvm::SmallVector<int64_t> VectorLayout::tileArrayImplicitShape(
  ...
  llvm::SmallVector<int64_t> tiles_shape = implicit_shape;
  tiles_shape[tiles_shape.size() - 2] = ceilDiv(
      offsets_[0].value_or(0) + implicit_shape[implicit_shape.size() - 2],
      vreg_slice[0]);
  tiles_shape[tiles_shape.size() - 1] = ceilDiv(
      offsets_[1].value_or(0) + implicit_shape[implicit_shape.size() - 1],
      vreg_slice[1]);
  return tiles_shape;
}
...
bool VectorLayout::generalizes(
  ...
  if (implicit_dim_ != other.implicit_dim_) {
    ...
    if (llvm::SmallVector<int64_t> implicit_shape = implicitShape(shape);
        !(implicit_shape[implicit_shape.size() - 2] == 1 &&
  ...
  if (tiling_ != other.tiling_) {
    ...
    if (!(shape.data() != nullptr && tiling_[1] == other.tiling_[1] &&
          tiling_[1] == target_shape[1] && tiling_[0] == other.tiling_[0] &&
          tiling_[0] == target_shape[0] &&
          offsets_[1].value_or(0) + shape[shape.size() - 1] <=
              target_shape[1] &&
          offsets_[0].value_or(0) + shape[shape.size() - 2] <=
