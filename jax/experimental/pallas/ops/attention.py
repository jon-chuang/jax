...
def mha_forward_kernel(
    ...
    o_ref: jax.Array,  # Output
    *residual_refs: jax.Array,  # Residual outputs
    sm_scale: float,
    causal: bool,
    block_q: int,
    block_d: int,
...
def segment_mask(
  ...
  if q_segment_ids is None or kv_segment_ids is None:
    return jnp.array(True).astype(jnp.bool_)
  # [B, T, 1] or [T, 1]
...
def mha(
  ...
  if segment_ids is None:
    in_specs += [None]   # type: ignore[list-item]
  else:
    in_specs.append(pl.BlockSpec(lambda _, j, k: (j, 0), (None, seq_len)))
...
def _mha_forward(
  ...
  if segment_ids is None:
    ...
    in_specs += [pl.BlockSpec(lambda _, j, k: (j, 0), (None, seq_len))]
