...
def _clamp_lowering_rule(ctx: LoweringRuleContext, min, operand, max):
  ...
  res_ctx = ctx.replace(avals_in=(min_aval, operand_aval), block_shapes=None)
  res = arith.SelectOp(
      lowering_rules[lax.gt_p](res_ctx, min, operand),
      min, operand).result
  return arith.SelectOp(
      lowering_rules[lax.lt_p](
          ctx.replace(avals_in=(res_ctx.avals_out[0], max_aval),
                      block_shapes=None),
          res, max),
      res, max).result
